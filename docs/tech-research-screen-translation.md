...
- **Windows App SDK の新 Text Recognition API** は Windows.Media.Ocr より高速・高精度とMicrosoftは主張するが、**NPU搭載機(Copilot+ PC)専用**であり、一般PCでは使えない(出典: [Microsoft Learn](https://learn.microsoft.com/en-us/windows/ai/apis/text-recognition))。将来のオプション扱いが妥当。
...
**確度の高い傾向**(複数ソース一致): ①従来型NMT系API(Google/Azure)はLLM系より一桁速い ②DeepLは品質面の評価が高いがレイテンシは大きめ ③コストはAzureが最安・無料枠最大。

### 3.2 ローカル(オフライン)翻訳エンジン

| エンジン/モデル | サイズ | 速度 | 品質 | ライセンス |
|------------------|--------|------|------|-----------|
| Bergamot / Firefox Translations (Marian NMT) | **約17MB/言語ペア**(量子化student) | 教師比37倍(1CPUコアで実用速度) | BLEU低下は教師比約1.8pt。一部言語ペアでクラウド同等というMozilla自己評価 | MPL系(要確認) |
| OPUS-MT (+ CTranslate2) | 小型 | 高速(int8量子化で更に向上) | ドラフト品質のベースライン向き | **Apache-2.0** |
| FuguMT (日英特化) | 約300MB | 高速 | Sugoi-v4同等との評価(フォーラム情報) | 要確認 |
| Sugoi-v4 | 約300MB(全体約1GB) | CTranslate2でint8量子化可 | 日英ゲーム翻訳で定評(**主方向は日→英**。英→日への適用可否は要検証) | 要確認 |
| NLLB-200 | 大 | 中 | 広カバレッジ | **CC-BY-NC-4.0(非商用)— 商用配布に制約** |
| ローカルLLM (Qwen 32B級) | 数十GB | 遅 | 英→日はクラウド品質の80〜90%程度との評価 | — |

出典: [Mozilla Hacks](https://hacks.mozilla.org/2022/06/training-efficient-neural-network-models-for-firefox-translations/) / HuggingFaceフォーラム / insiderllm.com / AMD ROCmブログ

**確度の高い傾向**: ①翻訳専用NMTは汎用LLMより小型・高速で、翻訳タスク限定なら品質も遜色ない(複数ソース一致) ②Marian系(Bergamot)の蒸留+量子化モデルはCPUのみでリアルタイム翻訳可能(Firefoxに実搭載済みという強い実証) ③**英→日はローカルモデルにとって難しい言語ペア**であり、品質は実測評価が必要(推測を含む)。

### 3.3 ローカル vs クラウドの使い分け(推奨)

- **速度最重要**という要件から、**ネットワーク往復(数十〜数百ms)自体を排除できるローカルNMTを第一候補**とする。ただし英→日の品質は事前検証が必須。
- クラウドは「高品質モード」として併存させる。エンジンは既存OSS 3種すべてが採用する**プラガブル構成**(切り替え可能な抽象化レイヤー)にする。
- **キャッシュが最大の高速化要素**(推測だが定石): 画面翻訳では同じ文が繰り返し出現するため、原文→訳文のLRUキャッシュで実効レイテンシをほぼゼロにできる。
...
- **検証済み(3票一致)**: UiPathのスクレイピング方式比較(FullText 10/10・100%、Native 8/10・座標可・GDI限定)。
- **出典引用あり・未検証**: 本レポートの大部分の主張は一次ソース(Microsoft Learn、各OSSのREADME、Mozilla公式ブログ)からの引用付き抽出だが、ワークフローの敵対的検証フェーズがセッションリミットで大半未完了(22/25件が未検証)。一次ソース由来のため確度は高いが、実装前に該当ドキュメントの現物確認を推奨。
- **ブログ由来の数値(要注意)**: クラウドAPIのレイテンシ・料金比較、Tesseract vs PaddleOCR速度比較は測定条件不明のブログ情報。**採用判断には自前ベンチマークが必須**。
- **知識ベースの推測(未出典)**: クリックスルーオーバーレイの実装詳細(WS_EX_*スタイル)、BitBltの黒画面問題、WinUI 3の透過制約、Sugoiの翻訳方向、PCOTの構成。
...
2. **PoC②(ローカル翻訳の品質・速度検証)**: CTranslate2 + OPUS-MT(またはBergamotの英日studentモデルの有無確認)で英→日の品質と速度を実測し、クラウドと比較する。
